...
test("province -> district -> station linked retrieval", async () => {
  const provinceRes = await request(app).post("/api/v1/province").send({ name: "Western", code: "WP" });
  ...
  const stationRes = await request(app)
    .post("/api/v1/police-station")
    .send({ name: "Colombo Police Station", code: "CMB-PS", district: districtRes.body._id });
  assert.equal(stationRes.status, 201);

  const listRes = await request(app).get(`/api/v1/police-station?provinceId=${provinceRes.body._id}`);
...
test("create/list tuk with filters", async () => {
  const province = await request(app).post("/api/v1/province").send({ name: "Central", code: "CP" });
  ...
  const tuk = await request(app).post("/api/v1/tuk").send({
  ...
  const districtFiltered = await request(app).get(`/api/v1/tuk?districtId=${district.body._id}`);
  assert.equal(districtFiltered.status, 200);
  assert.equal(districtFiltered.body.data.length, 1);

  const provinceFiltered = await request(app).get(`/api/v1/tuk?provinceId=${province.body._id}`);
...
test("create/list pings with time-window filters", async () => {
  const province = await request(app).post("/api/v1/province").send({ name: "Southern", code: "SP" });
  const district = await request(app)
    .post("/api/v1/district")
  ...
  const ping1 = await request(app).post("/api/v1/location-ping").send({
    tuk: tuk.body._id,
    latitude: 6.03,
    longitude: 80.21,
    pingedAt: oldTime
  ...
  const ping2 = await request(app).post("/api/v1/location-ping").send({
    tuk: tuk.body._id,
    latitude: 6.04,
    longitude: 80.22,
    pingedAt: newTime
  ...
  const list = await request(app).get(`/api/v1/location-ping?tukId=${tuk.body._id}&from=${encodeURIComponent(from)}`);
  assert.equal(list.status, 200);
  assert.equal(list.body.data.length, 1);

  const lastLocation = await request(app).get(`/api/v1/tuk/${tuk.body._id}/last-location`);
